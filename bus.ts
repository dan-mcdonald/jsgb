...
export function buildBus(bootRom: Uint8Array, cart: Cart, ppu: PPU, audio: Audio): [BusWrite, BusRead] {
  let iereg = 0;
  let bootRomDisable = false;
  const hram = new Uint8Array(0x7f);
  const wram = new Uint8Array(0x2000); // C000-DFFF
  const supportedPpuRegisters: Array<number> = [0xff40, 0xff42, 0xff43, 0xff44, 0xff47, 0xff48, 0xff49];
