...
function xor_r8(reg: R8): InstructionFunction {
  return function (cpu: CPU, _: Bus): number {
    xor(cpu, get8(cpu, reg));
    return 4;
  };
}

function xor_n8(val: number): InstructionFunction {
  return function (cpu: CPU, _: Bus): number {
    xor(cpu, val);
    return 8;
  };
...
function xor_at_HL(cpu: CPU, bus: Bus): number {
...
export function ldd_r8_at_r16(destAddrReg: R8, atRegSrc: R16.HL): InstructionFunction {
  return function (cpu: CPU, bus: Bus): number {
...
function ld_r8_at_r8(destReg: R8, srcAddrReg: R8): InstructionFunction {
  return function(cpu: CPU, bus: Bus): number {
    const addr = 0xff00 + get8(cpu, srcAddrReg);
    const val = bus.readb(addr);
    set8(cpu, destReg, val);
    return 8;
  };
}
...
function dec_at_HL(cpu: CPU, bus: Bus): number {
  const addr = get16(cpu, R16.HL);
  const oldVal = bus.readb(addr);
  const newVal = dec(cpu, oldVal);
  bus.writeb(addr, newVal);
...
function sub_at_HL(cpu: CPU, bus: Bus): number {
  const addr = get16(cpu, R16.HL);
  const val = bus.readb(addr);
  sub(cpu, val);
  return 8;
...
function rr_at_HL(cpu: CPU, bus: Bus): number {
  const addr = get16(cpu, R16.HL);
  const val = bus.readb(addr);
  const newVal = rr(cpu, val);
  bus.writeb(addr, newVal);
  return 16;
}
...
function srl_r8(reg: R8): InstructionFunction {
  return function(cpu: CPU, _: Bus): number {
    const oldVal = get8(cpu, reg);
...
function adc_r8(reg: R8): InstructionFunction {
  return function (cpu: CPU, _: Bus): number {
    adc(cpu, get8(cpu, reg));
    return 4;
  };
}

function adc_at_HL(cpu: CPU, bus: Bus): number {
  const addr = get16(cpu, R16.HL);
  const val = bus.readb(addr);
  adc(cpu, val);
  return 8;
...
function sbc_at_HL(cpu: CPU, bus: Bus): number {
  const addr = get16(cpu, R16.HL);
  const val = bus.readb(addr);
  sbc(cpu, val);
  return 8;
...
function sbc_r8(reg: R8): InstructionFunction {
  return function (cpu: CPU, _: Bus): number {
    sbc(cpu, get8(cpu, reg));
    return 4;
  };
}

function sbc_n8(val: number): InstructionFunction {
  return function (cpu: CPU, _: Bus): number {
    sbc(cpu, val);
    return 8;
  };
  ...
  switch (opcode) {
    ...
    case 0x3A:
      return {
        length,
        text: "ldd  a,(hl)",
        exec: ldd_r8_at_r16(R8.A, R16.HL),
      };
    case 0x3B:
      return {
        length,
    ...
    case 0x40:
      return {
        length,
        text: "ld   b,b",
        exec: ld_r8_r8(R8.B, R8.B),
      };
    case 0x41:
      return {
        length,
        text: "ld   b,c",
        exec: ld_r8_r8(R8.B, R8.C),
      };
    case 0x42:
      return {
        length,
        text: "ld   b,d",
        exec: ld_r8_r8(R8.B, R8.D),
      };
    case 0x43:
      return {
        length,
        text: "ld   b,e",
        exec: ld_r8_r8(R8.B, R8.E),
      };
    case 0x44:
      return {
        length,
        text: "ld   b,h",
        exec: ld_r8_r8(R8.B, R8.H),
      };
    case 0x46:
      return {
        length,
    ...
    case 0x4B:
      return {
        length,
        text: "ld   c,e",
        exec: ld_r8_r8(R8.C, R8.E),
      };
    case 0x4D:
      return {
        length,
        text: "ld   c,l",
        exec: ld_r8_r8(R8.C, R8.L),
      };
    case 0x4E:
      return {
        length,
        text: "ld   c,(hl)",
        exec: ld_r8_at_r16(R8.C, R16.HL),
      };
    case 0x4F:
      return {
        length,
    ...
    case 0x58:
      return {
        length,
        text: "ld   e,b",
        exec: ld_r8_r8(R8.E, R8.B),
      };
    case 0x59:
      return {
        length,
        text: "ld   e,c",
        exec: ld_r8_r8(R8.E, R8.C),
      };
    case 0x5A:
      return {
        length,
        text: "ld   e,d",
        exec: ld_r8_r8(R8.E, R8.D),
      };
    case 0x5B:
      return {
        length,
        text: "ld   e,e",
        exec: ld_r8_r8(R8.E, R8.E),
      };
    case 0x5C:
      return {
        length,
        text: "ld   e,h",
        exec: ld_r8_r8(R8.E, R8.H),
      };
    case 0x5D:
      return {
        length,
        text: "ld   e,l",
        exec: ld_r8_r8(R8.E, R8.L),
      };
    case 0x5E:
      return {
        length,
    ...
    case 0x70:
      return {
        length,
        text: "ld   (hl),b",
        exec: ld_at_r16_r8(R16.HL, R8.B),
      };
    case 0x71:
      return {
        length,
        text: "ld   (hl),c",
        exec: ld_at_r16_r8(R16.HL, R8.C),
      };
    case 0x72:
      return {
        length,
    ...
    case 0x74:
      return {
        length,
        text: "ld   (hl),h",
        exec: ld_at_r16_r8(R16.HL, R8.H),
      };
    case 0x75:
      return {
        length,
        text: "ld   (hl),l",
        exec: ld_at_r16_r8(R16.HL, R8.L),
      };
    case 0x76:
      return {
        length,
